...
CalePOS — offline-first, multi-branch point of sale for retail and meat counters (₱), built for the Philippines with BIR fiscal-readiness controls (sequential invoice numbering, immutable sales records, void/audit logging). Proprietary, authored by Jazper Bustria — see `LICENSE`.
...
## Tooling & Context Efficiency

Use the project's installed tooling to minimize unnecessary context usage while preserving correctness.

### Codebase exploration — Graphify

Prefer Graphify for understanding relationships, dependencies, call paths, architecture, and locating relevant code before broad text searches.

Use the Graphify skill/tools when available rather than repeatedly scanning the repository with Grep/Glob/Read.

The Graphify graph may become stale after significant changes; use the Graphify update/watch workflow when needed.

Do not use Graphify as a substitute for reading the actual implementation. Once relevant files/functions are identified, inspect the source before editing.

Do not repeatedly rebuild or query the entire graph for a localized task.

### Library and API documentation — Context7

Prefer Context7 when current or version-specific documentation is needed for a framework, library, SDK, package, or API.

Use Context7 instead of relying on remembered API signatures when authoritative current documentation is available.

Resolve the relevant library first, then query only the documentation needed for the current task.

Do not use Context7 for repository-local questions that Graphify/source inspection can answer.

Do not fetch large amounts of documentation when a focused query is sufficient.

### Large tool output — Context Mode Usage

When Context Mode is available, use it automatically for context-heavy operations:

- Large command output, logs, test/build output, or generated data.
- Large files or documents when only specific information is needed.
- Web/document content that is substantially larger than the relevant answer.
- Batch operations where returning every result to the main context would be wasteful.
- When processing or searching previously indexed content.

Prefer `ctx_*` tools over normal Bash/file-output tools when the expected result is large enough that sending the entire output into the main context would be wasteful.

For small, focused operations, use the normal tool directly. Do not use Context Mode when its overhead is greater than the context saved.

When using Context Mode, retrieve only the relevant information needed for the current task rather than returning the entire indexed result.

### Terminal output — RTK

RTK is configured globally for Claude Code and should transparently optimize supported terminal commands.

Do not manually prefix commands with rtk when the Claude Code hook can handle the command normally.

Prefer normal commands such as git status, git diff, git log, npm run lint, and npm run build; let the RTK hook optimize their output.

Do not rely on RTK for source-code exploration that is better handled by Graphify or direct file inspection.

### Tool selection rules

Choose the smallest tool/workflow that can answer the question correctly:

Repository relationships/architecture: Graphify first, then inspect relevant source.

Current external library/API documentation: Context7 first.

Large output/logs/web content: Context Mode.

Normal terminal commands: run them normally and let RTK optimize supported output.

Exact small file or symbol: use direct Read/Grep when faster than another layer.

Browser/UI behavior: use the available browser/testing tool when actual runtime behavior must be verified.

Do not invoke multiple tools that solve the same problem unless the first result is insufficient. Tooling exists to reduce unnecessary context, not to add extra steps.
...
* Changes involving money, sales records, refunds, voids, inventory, pricing, taxes, invoice numbering, audit logs, or other fiscal behavior.
...
For changes involving authentication, authorization, RLS, payments, inventory, pricing, refunds, voids, taxes, invoice numbering, audit logs, or sales records:
...
  computation, invoice numbering). These need a heads-up before deploy.
