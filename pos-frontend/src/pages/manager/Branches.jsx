          ...
          return (
            <TableCard key={branch.id} className="max-h-none p-5">
              <div className="mb-3 flex items-start justify-between gap-2">
                ...
                <span className={`rounded-full px-2 py-1 text-[10px] font-bold ${branch.is_active ? 'bg-brand-success-bg text-brand-success-text' : 'bg-brand-danger-bg text-brand-danger'}`}>
              ...
              <div className="flex gap-2">
                <Link className="inline-flex h-10 items-center rounded-[5px] bg-brand-gold px-4 text-xs font-bold text-brand-dark no-underline" to={`/manager/branches/${branch.id}`}>
                  Open dashboard
                </Link>
        ...
        <div className="fixed inset-0 z-[5] grid place-items-center bg-[#202426aa]">
          ...
          >
            ...
            <div className="mt-4 flex justify-end gap-2">
              ...
              <PrimaryButton compact type="submit">Save</PrimaryButton>
            </div>
          </form>
        </div>
      )}
