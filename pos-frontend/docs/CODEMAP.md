...
Same SPA, wrapped as a native Android app — no separate mobile codebase, no platform-specific
React code. `capacitor.config.json` (`appId: com.calepos`, `webDir: dist`) points Capacitor at
the Vite production build; the `android/` directory is a generated Android Studio project
(gradle wrapper, `MainActivity.java`, res/) that is committed (its own `android/.gitignore`
excludes `build/` output — confirmed nothing under `android/**/build/` is tracked).

| Piece | File |
|-------|------|
| Capacitor config (webDir, plugins) | `capacitor.config.json` |
| Native Android project | `android/` (generated by `npx cap add android`, committed) |
| Source icon/splash | `resources/icon.png`, `resources/splash.png`, `resources/splash-dark.png` |
| Generated per-density assets | `resources/android/**` (gitignored — regenerate, don't hand-edit) |

Installed plugins: `@capacitor/android`, `@capacitor/app`, `@capacitor/core`,
`@capacitor/haptics`, `@capacitor/keyboard`, `@capacitor/status-bar` (+ `@capacitor/cli` dev
dep). None are called from `src/` yet — the only active plugin behavior is declarative, via
`capacitor.config.json`'s `SystemBars` block (status bar hidden, `insetsHandling: css`,
edge-to-edge margins disabled). Rebuild flow is manual (no `package.json` script wired up):
`npm run build` → `npx cap sync android` → open `android/` in Android Studio → build/run APK.

`DesktopModeHint` (the "you're on a small screen" banner) was removed from `App.jsx` entirely
as part of this work, not just suppressed on the native build — it no longer renders on any
platform. The component file (`src/components/shared/DesktopModeHint.jsx`) is now dead code;
decide whether to delete it or repurpose it before relying on it again.
...
**Invoice number: never generated on-device.** A client-computed number is only atomic within
...
sets `invoiceNumber: null` unconditionally and checkout never awaits sync — the receipt prints
immediately with the sale's local `id` as a PENDING reference (`buildReceipt`, `receipt.js`)
instead. `Cart.jsx`'s `waitForRealInvoiceNumber` polls for the real number too, but strictly
...
"Sale complete" screen or the print itself. The real invoice number is assigned only when this sale's `COMPLETE_SALE` queue item reaches
the server, inside `complete_sale(...)` (`migrate_complete_sale_rpc.sql`, function bodies updated by `migrate_rename_or_to_invoice.sql`) — `select … for
update` row-locks the branch's `invoice_next` counter (via `allocate_invoice_number`/
`reserve_invoice_number`, called as plain function calls from inside `complete_sale`), so
...
client-supplied number reconciled server-side via `reserve_invoice_number` — still exists for
compatibility but nothing calls it with a number anymore; `completeSale`'s
`invoiceNumber`/`clientInvoiceNumber` parameter is always null now, which routes it straight to
`allocate_invoice_number`. Once the push succeeds the local pending row is deleted and the next
pull brings down the authoritative server row (with its real invoice number) — the same
...
detail modal ("Print receipt") shows the real invoice number once synced; `item.invoiceNumber || `Pending ·
...
`complete_sale(...)` (`migrate_complete_sale_rpc.sql`) does till check → invoice allocation →
...
multi-step flow — assert_till_open + allocate/reserve_invoice_number → insert `transactions` →
...
of the existing invoice; see Manager tracking).
...
- `src/pages/manager/BranchDashboard.jsx`
  ...
  - **Recent receipts** open `TransactionDetailModal` with **Print receipt** — browser reprint
    of the existing invoice (`buildReceipt` + `receiptPrinter.printReceipt`); uses the *viewed*
    branch fiscal header (`fetchBranchFiscalHeader`), does not mint a new invoice number, and is not
...
- **BranchDashboard** live updates (`day_ends` / `cash_drawer_entries` / `refund_requests`)
  ...
  product bootstrap.
- **All three dashboards are now live**, via the same `useLiveData` broadcast+poll pattern
  (`pos:branch:<id>:operations`, 15s poll fallback; `manager/Overview.jsx` instead
  subscribes to `NETWORK_OPERATIONS_TOPIC` = `pos:network:operations`, which
  `broadcast_pos_event()` also fires on every branch's `operations`-channel change).
  `manager/Overview.jsx`'s live refresh is silent (`loadMetrics({ silent: true })`) — no
  loading skeleton, keeps the chart-point drill-down (`selectedPointIndex`) — a `periodRef`
  guard drops a stale response if the user switches period mid-flight. Supervisor
  `Dashboard.jsx`'s `cashImpact`/`auditEvents` used to be one-shot `useEffect`s that never
  refreshed after mount (Net sales/Orders/Sales performance looked live because they read
  `useInventoryStore.transactions`, already kept fresh by Shell's global
  `useBranchOperationsLive`, while cash impact/audit silently went stale) — now on the same
  `useLiveData` hook as the other two.
...
src/offline/realtime.js  subscribeBroadcast (+ subscribeTable for promo child tables)
   ...
   │     ├─► manager/Promos.jsx same postgres_changes trio, branch-scoped → refreshActive
   │     │     (supervisor's pending request approved/rejected elsewhere reaches their
   │     │     open Promos.jsx without a reload)
...
distinct utility face for tabular money/qty/invoice-number figures, via `moneyClass`/`<Money>`
...
| Offline invoice reserve on sync | `migrate_offline_or_reserve.sql` (`reserve_invoice_number`) |
| OR → Sales Invoice rename (columns, RPCs, immutability trigger) | `migrate_rename_or_to_invoice.sql` |
...
| Atomic checkout RPC (till + invoice + transaction + items + stock + audit in one transaction) | `migrate_complete_sale_rpc.sql` |
...
login. `allocate_invoice_number`, `reserve_invoice_number`, `void_sale_secure`, `refund_sale_items`,
...
the invoice number (e.g. `SI-00000001`) in one chunked lookup and blanks any other bare UUID before the row reaches
...
- **Bulk import price-change logging** — `commitCatalogImport`'s update-existing-row branch calls `recordPriceChange` when the imported price differs from the row's price before the import, matching every other price-editing path.
- **Branch Inventory import only ever restocks quantity.** `commitInventoryImport`'s existing-product branch (`action: 'restock'`, from `buildImportPreview`) writes **only** the on-hand delta via `record_stock_movement` — it never calls `writeProductRow` or `recordPriceChange` for that row, so name/SKU/barcode/category/price/pricingMode/discountEligible on the sheet are ignored even if they differ from the live product. `describeImportFieldChanges({ restockOnly: true })` keeps the preview in sync — it only ever shows the stock delta for a restock line, never a phantom field diff the commit won't actually apply. To edit those other fields in bulk, use Manager Network Catalog's import (`commitCatalogImport`, which does cascade identity/price) or `Products.jsx` for one branch. This mirrors `updateProductRow`'s existing "only touch what's explicitly passed" guard just above.
