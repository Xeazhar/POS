-- Fix: manager_overview_metrics() returned 'revenue' as gross total_amount (no refund
-- subtraction), while its own 'netSales' key correctly netted total_amount - refunded_amount.
-- Manager Overview's headline "Revenue" KPI (and its period-comparison delta) therefore
-- overstated actual revenue by however much had been refunded, disagreeing with
-- BranchDashboard.jsx's "Revenue today"/"Net sales" figure, which was always netted.
--
-- Fix: 'revenue' now equals 'netSales' (drop the separate un-netted v_revenue variable —
-- there is no longer a reason for the two figures to differ). Supersedes
-- migrate_network_manager_overview.sql's function body; everything else is unchanged.
...
comment on function public.manager_overview_metrics(integer) is
  'Manager Overview: per-branch sales KPIs for the last N local days + today cash impact in one round trip. revenue = netSales (net of refunds) as of migrate_fix_manager_overview_revenue_net.sql.';
