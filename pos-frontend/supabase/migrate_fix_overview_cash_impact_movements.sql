-- Fix: manager_overview_metrics()'s cash impact (pickup/paidOut/changeFund, and the
-- Overview page's network-wide "Cash in / out" card built from it) only summed
-- cash_drawer_entries — the legacy petty-cash table. It never read cash_movements, the
-- newer POS -> Open Drawer dual-control ledger (migrate_cash_movements.sql), so a petty
-- cash paid-out or pickup recorded through Open Drawer silently never moved this figure,
-- even once approved/confirmed, while the same amount correctly showed up in DayEnd.jsx,
-- BranchDashboard.jsx's Cash drawer log, and DayEndClosingDetail.jsx (all three already
-- merge both tables). api.fetchBranchCashImpact (the pre-RPC fallback) had the identical
-- gap — fixed alongside this migration in the same commit.
--
-- Fix mirrors DayEnd.jsx's movePaidOutTotal/movePickupTotal/moveCashInTotal exactly: only
-- cash_movements rows whose status is in CASH_MOVEMENT_COUNTING_STATUSES ('approved',
-- 'remote_approved', 'self_recorded', 'confirmed', 'flagged_for_investigation') have
-- actually left/entered the drawer — a still-pending request is not money movement yet.
-- 'petty_cash' -> paid out, 'pickup' -> pickup, 'cash_in' -> change fund. 'opening_float'
-- rows are intentionally excluded, same as the client: that float is already counted via
-- staff_shifts.starting_cash (v_change_fund's first select) and a cash_movements
-- 'opening_float' row is just its audit trail, not additional money.
--
-- Prerequisite: migrate_fix_overview_cash_impact_carry.sql (supersedes its body; only the
-- Cash impact block changes — sales KPIs, low stock, menu on/off are unchanged).
-- Safe to re-run.
...
comment on function public.manager_overview_metrics(integer) is
  'Manager Overview: per-branch sales KPIs for the last N local days + today cash impact (cash_drawer_entries + cash_movements) in one round trip.';
