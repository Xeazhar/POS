...
|-------------|-------------------|
| Sequential invoice numbering | Server-allocated `SI-########` per branch (`allocate_invoice_number`) |
...
| Daily sales reports | Z / X / BIR summary from live invoice range + totals; day-end cash close |
| Data backup | Manager → **Fiscal Data Backup** JSON export; rely on Supabase project backups too |
| Receipt / invoice printing | Browser print with TIN, invoice number, business name, MIN, SN — thermal printer hook ready |
